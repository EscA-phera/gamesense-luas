...
local wesync_enabled = ui.new_checkbox("LUA", "A", "WeSync")
local yaw_base = ui.new_multiselect("LUA", "A", "Yaw base", "Crosshair", "At targets")
local yaw_base_modifier = ui.new_multiselect("LUA", "A", "Yaw modifier", "Manual", "Freestanding", "Anti-bruteforce")
local manual_invert_key = ui.new_hotkey("LUA", "A", "Manual key")
local manual_indicator = ui.new_hotkey("LUA", "A", "Manual AA indicator")
local manual_indicator_offset = ui.new_hotkey("LUA", "A", "Indicator offset", 0, 100, 50)
local manual_indicator_color = ui.new_color_picker("LUA", "A", "Indicator color", 40, 150, 255, 255)
local modifier_limit = ui.new_multiselect("LUA", "A", "Modifier limit", 0, 180, 90)
local desync_type = ui.new_combobox("LUA", "A", "Desync type", "Static", "Jitter", "Edge", "Reverse Edge")
local fake_limit = ui.new_slider("LUA", "A", "Fake yaw limit", 0, 60, 45)

local ref_yaw_base = ui.reference("RAGE", "Anti-aimbot angles", "Yaw base")
local ref_yaw_type, ref_yaw_offset = ui.reference("RAGE", "Anti-aimbot angles", "Yaw")
local ref_jitter_type, ref_jitter_offset = ui.reference("RAGE", "Anti-aimbot angles", "Yaw jitter")
local ref_byaw_type, ref_byaw_offset = ui.reference("RAGE", "Anti-aimbot angles", "Body yaw")
local ref_fake_limit = ui.reference("RAGE", "Anti-aimbot angles", "Fake yaw limit")
local ref_lby_type = ui.reference("RAGE", "Anti-aimbot_angles", "Lower body yaw target")
...
local function handle_ui()
    local main_state = ui.get(wesync_enabled)
    local manual_state = ui.get(yaw_base_modifier) == "Manual"
    
    ui.set_visible(yaw_base, main_state)
    ui.set_visible(yaw_base_modifier, main_state)
    ui.set_visible(manual_invert_key, main_state and manual_state)
    ui.set_visible(manual_indicator, main_state and manual_state)
    ui.set_visible(manual_indicator_offset, main_state and manual_state)
    ui.set_visible(manual_indicator_color, main_state and manual_state)
    ui.set_visible(modifier_limit, main_state)
    ui.set_visible(desync_type, main_state)
    ui.set_visible(fake_limit, main_state)
end

local manual_aa_direction = -1
local last_direction_changed = 0
local screen_width, screen_height = client.screen_size()

local function on_paint()
    if not (ui.get(wesync_enabled) or ui.get(yaw_base_modifier) == "Manual" or ui.get(manual_indicator)) then return end

    local local_player = entity.get_local_player()
    if local_player == nil or not entity.is_alive(local_player) then return end

    local r, g, b = ui.get(manual_indicator_color)
    local offset = ui.get(manual_indicator_offset)

    renderer.text(
        ui.get(manual_indicator_offset) * manual_aa_direction,
        screen_height / 2,
        r,
        g,
        b,
        "c+",
        0,
        manual_aa_direction == -1 and "<" or ">"
	)
	
	renderer.text(
		ui.get(manual_indicator_offset) * manual_aa_direction * -1,
        screen_height / 2,
        160,
        160,
        160,
...
local function on_run_command(event)
    if not ui.get(wesync_enabled) then return end

    local local_player = entity.get_local_player()
    if local_player == nil or not entity.is_alive(local_player) then return end

    if ui.get(yaw_base) == "Crosshair" then
        ui.set(ref_yaw_base, "Local view")
    elseif ui.get(yaw_base) == "At targets" then
        ui.set(ref_yaw_base, "At targets")
    end

    if ui.get(yaw_base_modifier) == "Manual" then
        if ui.get(manual_invert_key) and globals.curtime() - last_direction_changed > 0.1 then
            manual_aa_direction = manual_aa_direction * -1
        end

        ui.set(ref_yaw_type, "180")
        ui.set(ref_yaw_offset, ui.get(manual_indicator_offset) * manual_aa_direction)
    elseif ui.get(yaw_base_modifier) == "Freestanding" then
    elseif ui.get(yaw_base_modifier) == "Anti-bruteforce" then
    end
end
